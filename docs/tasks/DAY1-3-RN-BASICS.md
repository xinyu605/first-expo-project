...
- [x] **React Navigation 入門**
  - 了解 Stack Navigator 基本設定
  - 掌握 Tab Navigator 的使用
  - 理解導航參數傳遞

#### 📋 React Navigation 入門 — 任務拆解

任務設計與實作清單見 OpenSpec change：`openspec/changes/day1-3-react-navigation-intro/`（[proposal](../../openspec/changes/day1-3-react-navigation-intro/proposal.md)、[tasks](../../openspec/changes/day1-3-react-navigation-intro/tasks.md)）。

**1. Stack Navigator 基本設定**

- [x] 能說明 Stack Navigator 的用途與基本設定方式（對應 `app/_layout.tsx`）
- [x] 能修改 Stack.Screen 的 `options`（如 `title`、`headerShown`）並觀察效果
- [x]（可選）能說明 Expo Router 的 Stack 與 React Navigation native-stack 的關係
- [x] 能說明 unstable_settings 的 `initialRouteName` 的用途與基本設定（對應 `app/_layout.tsx`）

**2. Tab Navigator 的使用**

- [x] 能說明 Tab Navigator 的用途與基本設定（對應 `app/(tabs)/_layout.tsx`）
- [x] 能修改 tab 的 title、icon 或共用樣式（如 `tabBarActiveTintColor`）
- [x] 能說明 `tabBarButton: HapticTab` 在專案中的用途

**3. 導航參數傳遞**

- [x] 能說明導航參數如何從列表頁傳到明細頁（Expo Router 的 params / searchParams）
- [x] 能實作「列表 → 明細」導航並在明細頁讀取參數
- [x] 能實作從明細頁返回列表（`router.back()` 或 header 返回鈕）

完成上述任務後，可一併勾選實作任務中的「明細頁面」與「返回導航」。
...
- [ ]（可選）能解釋 Stack / Tab 在 app 中的角色，以及導航參數如何傳遞（見 React Navigation 入門任務）
